...
console.log(`Èñõîäíîå ðàñïîëîæåíèå: ${start.x}:${start.y}`);
console.log(`Òåêóùåå ðàñïîëîæåíèå: ${finish.x}:${finish.y}`);
...
items.set('Èãðîê', player);
items.set('Ïåðâàÿ ìîíåòà', new Actor(new Vector(10, 10)));
items.set('Âòîðàÿ ìîíåòà', new Actor(new Vector(15, 5)));
...
function status(item, title) {
  ...
  if (player.isIntersect(item)) {
    console.log(`Èãðîê ïîäîáðàë ${title}`);
  }
}
...
const goldCoin = new MyCoin('Çîëîòî');
const bronzeCoin = new MyCoin('Áðîíçà');
const player = new Actor();
...
if (level.noMoreActors('coin')) {
  console.log('Âñå ìîíåòû ñîáðàíû');
  console.log(`Ñòàòóñ èãðû: ${level.status}`);
}

const obstacle = level.obstacleAt(new Vector(1, 1), player.size);
if (obstacle) {
  console.log(`Íà ïóòè ïðåïÿòñòâèå: ${obstacle}`);
}

const otherActor = level.actorAt(player);
if (otherActor === fireball) {
  console.log('Ïîëüçîâàòåëü ñòîëêíóëñÿ ñ øàðîâîé ìîëíèåé');
}*/
...
class LevelParser {
	...
	createGrid(strings) {
		let grid = []; // ìàññèâ, êîòîðûé íàäî ñîçäàòü
		...
		for (let i = 0; i < strings.length; i++) {
			for (let j = 0; j < strings[i].length; j++) {
			  if(strings[i][j]==='@' || strings[i][j]==='o' || strings[i][j]==='=' || strings[i][j]==='|' || strings[i][j]==='v') {
				let actor = this.actorFromSymbol(strings[i].charAt(j)); //ïîëó÷àåì êîíñòðóêòîð îáúåêòà ïî ñèìâîëó
				if (actor!==undefined && typeof actor === 'function') {
				let item = new Actor(new Vector(i, j)); // ñîçäàåì íîâûé äâèæóùèéñÿ îáúåêò ñ âåêòîðîì
